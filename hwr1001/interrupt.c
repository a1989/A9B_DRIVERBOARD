...
uint16_t iTaskTimCount = 0;		//用于记录执行事件定时器的中断次数
...
void HAL_UART_RxCpltCallback (UART_HandleTypeDef* huart) //串口中断回调函数
{
	...
	if (huart->Instance == huart1.Instance) //串口1调试串口
	{
		...
		if (uart1_rxbuff[uart1_rxdata_cnt] == '\n')
		{
			...
			else if ((uart1_rdata[0] == UARTI_TEST_DATA_FRAME_START) && (uart1_rdata[1] == UARTI_TEST_DATA_FRAME_SECOND)) //接收到的是串口调试数据
			{
				memset (UARTI_DEBUG_DATA, 0, 256);
				for (i=0; i < uart1_debug_data_len; i++) //将数据填充进入数组
				{
					UARTI_DEBUG_DATA[i] = uart1_rdata[i + 4];
				}
...
{
	static uint32_t iTimTaskCount = 0;
	
	if (htim->Instance == htim3.Instance) //tim3 interrupt
	{
		iTaskTimCount++;
		if ((iTaskTimCount % led_task_cnt ) == 0)
		{
			led_display_cnt_flag = 1;    // 1s 用于打印计时500ms
		}
		if ((iTaskTimCount % can_send_data_task_cnt) == 0)
		{
			can_send_data_cnt_flag = 1;    //100ms发送一次
		}
		/*
		if ((iTaskTimCount % location_write_task_cnt) == 0)
		{
			location_write_cnt_flag = 1;    //1200ms发送一次
		}
		*/
		if ((motor_limit_flag == 1) || (motor_limit_flag == 2))
		{
			if ((iTaskTimCount % motor_limit_cnt) == 0)
			{
				motor_limit_cnt_flag = 1;
			}
		}
		
		if (iTaskTimCount > 65530)
		{
			iTaskTimCount = 0;
		}
	...
	if (htim->Instance == htim2.Instance) //tim2 interrupt
	{
		if (htim->Instance->CR1 & 0x0010) //小心注意
		{
			GetEncoder.rcnt3 -= 1;
